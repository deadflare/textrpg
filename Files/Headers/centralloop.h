...
int centralloop(){
	...
	while(true){
		...
		cin >> action;
			if(action == "0" || action == "q"){		//Confirm player meant to quit and then quit/cancel.
				string quitconfirm;
				cout << "\nQuit?\n1\tyes\n2\tno\n>";
				cin >> quitconfirm;
			...
			else if(action == "1"){Player.printstats();}
			...
			else if(action == "3"){shop();}
			else if(action == "4"){Player.inventoryloop();}
			else if(action == "5"){
